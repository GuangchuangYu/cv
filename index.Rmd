...
cp2012 <- function() {
  cat("\n\n::: aside\n\n")
  cat("![](figures/2021年中国科技论文统计报告发布会-高被引论文-截屏.png){width=90%}\n\n")
...
cp2021 <- function() {
  ...
  clusterProfiler4_awards <- "Honored with the Innovation Best Paper, Most Read, Most Download Awards."

  glue(
    "+ {clusterProfiler2021}, the featured article in 2021-08-24's website banner of Cell Press.\n+ {clusterProfiler4_awards}" 
  ) %>% print()

  cat("\n\n:::\n\n")
}

cp2024innovation <- function() {
  cat("\n\n::: aside\n\n")
  # cat("![](figures/smallmethod2025.png){width=100%}\n\n")
  paper <- "Yu (2024)"
  url <- "https://doi.org/10.1016/j.xinn.2024.100722"

  paper2 <- glue("[{paper}]({url})")

  glue(
    "+ {paper2}, published in The Innovation, was honored with the Best Paper Award." 
  ) %>% print()
...
sm2025 <- function() {
  ...
  url <- "https://doi.org/10.1002/smtd.202401107"

  paper2 <- glue("[{paper}]({url})")

  url2 <- "https://onlinelibrary.wiley.com/doi/10.1002/smtd.202570033"

  frontispiece <- glue("[frontispiece]({url2})")
  glue(
    "+ {paper2}, published in Small Methods, was featured on the issue's {frontispiece}." 
  ) %>% print()
...
til2025award <- function() {
  cat("\n\n::: aside\n\n")
  paper <- "Luo et al. (2024)"
  url <- "https://doi.org/10.59717/j.xinn-life.2024.100105"

  paper2 <- glue("[{paper}]({url})")
  glue(
    "+ {paper2}, published in The Innovation Lief, was honored with the Best Popular Award." 
  ) %>% print()
...
build_position_from_df <- function(pos_df){
  ...
  # 定位：后一篇文章
  if (grepl("ReactomePA", pos_df$title) && grepl("Molecular BioSystems", pos_df$loc)) {
    ggtree2017()
  }

  if (grepl("Artificial intelligence for life sciences: A comprehensive guide and future trends", pos_df$title)) {
    til2025award()
  }

  # 定位：后一篇文章
  if (grepl("16S rRNA", pos_df$title) && grepl("Photodermatology", pos_df$loc)) {
    sm2025()
  }
  
  # 定位：后一篇文章
  if (grepl("clusterProfiler", pos_df$title) && grepl("Nature Protocols", pos_df$loc)) {
    cp2024innovation()
  }
...
::: aside
![](figures/wiley2025.png){width=100%}
